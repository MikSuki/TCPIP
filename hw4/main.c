...
#define STUD_ID "m103040056"

int TIMEOUT = 1000;
// char *interface = "enp2s0f5";
char *interface = "eno1";
uint16_t pid;
int seq = 0;
...
int main(int argc, char *argv[])
{
    ...
    inet_pton(AF_INET, get_my_ip(), &ip_src);

    char ip[16], netmask[16];
    strcpy(ip, get_my_ip());
    strcpy(netmask, get_my_netmask());
    // char *ip = "140.117.168.64";
    ...
    for (unsigned long i = first_ip; i <= last_ip; ++i)
    {
        ...
        // fill_iphdr(&packet->ip_hdr, str_cur_ip);
        // fill_icmphdr(&packet->icmp_hdr);
        // fill_cksum(&packet->icmp_hdr);
        // strcpy(packet->data, STUD_ID);
        ...
        // set check sum
        packet->icmp_cksum = 0;
        packet->icmp_cksum = checksum((uint16_t *)packet, ICMP_HEADER_SIZE);
        // set student ID
        icmp_req_buffer[8] = 'm';
        icmp_req_buffer[9] = '1';
        ...
        // sendto(sockfd, icmp_req_buffer, ICMP_HEADER_SIZE+19, 0, (struct sockaddr*)&dst, sizeof(dst));

        if (sendto(sockfd, packet, /*PACKET_SIZE*/8, 0, (struct sockaddr *)&dst, sizeof(dst)) < 0)
        {
            perror("sendto");
            exit(1);
        }
